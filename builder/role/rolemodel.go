func (m *RoleModel) Insert(data Role) (int64, error) {
	insert := builder.Eq{
        "id": data.Id,
...
func (m *RoleModel) List(page, pageSize int) ([]*Role, int64, error) {
	...
	err := mr.Finish(func() error {
		...
		query, args, err := builder.Dialect(builder.MYSQL).Select(roleRows).From(m.table).Where(where).Limit(pageSize, offset).ToSQL()
		if err != nil {
			return err
		}
...
func (m *RoleModel) InsertAll(list []*Role) error {
	insert := builder.Eq{
        "id": "",
        "brand_id": "",
        "name": "",
	...
	for i, data := range list {
		...
		if err != nil {
			logx.Errorf("called RoleModel InsertAll exec: %s", err)
			continue
		}
		if i != 0 && i%1000 == 0 {
			bulk.Flush()
...
func (m *RoleModel) Update(data Role) error {
	update := builder.Eq{
        "id": data.Id,
        "brand_id": data.BrandId,
        "name": data.Name,
