func (m *AccountDepartmentReModel) Insert(data AccountDepartmentRe) (int64, error) {
	insert := builder.Eq{
        "id": data.Id,
...
func (m *AccountDepartmentReModel) List(page, pageSize int) ([]*AccountDepartmentRe, int64, error) {
	where := builder.Eq{}

	var total int64
	resp := make([]*AccountDepartmentRe, 0)
	err := mr.Finish(func() error {
		offset := (page - 1) * pageSize
		query, args, err := builder.Dialect(builder.MYSQL).Select(accountDepartmentReRows).From(m.table).Where(where).Limit(pageSize, offset).ToSQL()
		if err != nil {
			return err
		}
...
func (m *AccountDepartmentReModel) InsertAll(list []*AccountDepartmentRe) error {
	insert := builder.Eq{
        "id": "",
        "brand_id": "",
        "department_id": "",
	...
	for i, data := range list {
		...
		if err != nil {
			logx.Errorf("called AccountDepartmentReModel InsertAll exec: %s", err)
			continue
		}
		if i != 0 && i%1000 == 0 {
			bulk.Flush()
...
func (m *AccountDepartmentReModel) Update(data AccountDepartmentRe) error {
	update := builder.Eq{
        "id": data.Id,
        "brand_id": data.BrandId,
        "department_id": data.DepartmentId,
